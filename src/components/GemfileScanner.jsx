...
const parseGemfileLock = (content) => {
  try {
    ...
    if (!packages.size) return { error: 'No packages found. Is this a valid Gemfile.lock?' }
    return { type: 'Gemfile.lock', packages: Array.from(packages.values()) }
  } catch (e) {
    ...
    const res = await fetch(OSV_API_BATCH_URL, {
      ...
      body: JSON.stringify({
        queries: chunks[i].map((p) => ({
          package: { name: p.name, ecosystem: 'RubyGems' },
          version: p.version,
        })),
      }),
    })
    ...
    <div className="bg-white border-2 border-black rounded-xl shadow-md p-4">
      ...
      {inputType === 'paste' ? (
        <textarea
          ...
          placeholder="Paste Gemfile.lock content here..."
          onChange={(e) => {
...
function ResultsTable({ results, filterText, setFilterText }) {
  ...
  const exportCsv = () => {
    ...
    link.download = 'gemfile-scan-results.csv'
    link.click()
...
export default function GemfileScanner() {
  ...
  const handleScan = async () => {
    setError('')
    setResults([])
    const parsed = parseGemfileLock(lockfileContent)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <h1 className="text-3xl font-bold mb-2">Gemfile.lock Scanner</h1>
      <p className="text-gray-600 mb-4">
        Paste or upload a Gemfile.lock to check for known vulnerabilities. Only gem names and versions are sent to OSV.dev.
      </p>
      <InputArea
